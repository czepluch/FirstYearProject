...
Whenever the user types something in one of the text fields for inputting addresses (in the \texttt{SearchPanel} class), the following happens:
\begin{itemize}
...
\begin{tikzpicture}
	\node [place, minimum size=2cm]	(SearchPanel)	at (0, 0) {SearchPanel}
		edge [in=-100, out=-80, loop] node[auto] {first validation} (SearchPanel);
	\node [place, minimum size=2cm]	(MainFrame)		at (0, 3) {MainFrame}
		edge [post] node [auto, swap] {update list} (SearchPanel);
	\node [place, minimum size=2cm]	(View)			at (5, 3) {View}
		edge [pre]	node [auto]	{input} (SearchPanel)
		edge [post] node [auto, swap] {if valid id} (MainFrame);
	\node [place, minimum size=2cm]	(AddressParser)	at (10, 3) {AddressParser}
		edge [pre] node [auto, swap] {parse input} (View);
	\node [place, minimum size=2cm]	(TernaryTrie)	at (10, -1) {TernaryTrie}
		edge [post, bend left=12] node [auto] {vertex id} (View)
		edge [pre, bend right=12] node [auto, swap] {if valid} (View);
\end{tikzpicture}
...
The validity checking is a follows:
\begin{itemize}
...
\begin{tikzpicture}
	\node [place, minimum size=2cm]	(View)			at (0,0)		{View}
		edge [in=-100, out=-80, loop] 	node [auto]	{updates} (View);
	\node [place, minimum size=2cm]	(Controller)		at (5,3)		{Controller}
		edge [pre, bend right=30]	node [auto, swap]	{vertex id}		(View)
		edge [post, bend left=30, very near end]	node [auto]	{MapLocation}		(View);
	\node [place, minimum size=2cm]	(Model)			at (10,0)		{Model}
		edge [pre, bend right=30]	node [auto, swap]	{vertex id}		(Controller)
		edge [post, bend left=30, dashed, very near start]	node [auto]	{return: MapLocation}		(Controller)
		edge [in=-100, out=-80, loop] 	node [auto]	{filters} (Model);
\end{tikzpicture}
